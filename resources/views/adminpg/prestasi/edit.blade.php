        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form id="form_update_prestasi" action="{{ route('adminpg.prestasi.update', $prestasi->id_prestasi) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="form-group">
                            <label for="judul" class="form-label">Judul</label>
                            <input placeholder="Judul..." id="judul" name="judul" class="form-default" type="text" value="{{ old('judul') ? old('judul') : $prestasi->judul }}" data-hs-combo-box-input="" />
                            <x-input-error :messages="$errors->get('judul')" class="mt-2" />
                        </div>
                        <div class="form-group">
                            <label for="tahun" class="form-label">Tahun</label>
                            <input placeholder="Tahun..." id="tahun" name="tahun" class="form-default" type="number" value="{{ old('tahun') ? old('tahun') : $prestasi->tahun }}" data-hs-combo-box-input="" />
                        ...
                        <div class="form-group">
                            <div class="mb-3">
                                ...
                                <textarea id="deskripsi" class="tinyMce" name="deskripsi">{{ old('deskripsi') ? old('deskripsi') : $prestasi->deskripsi }}</textarea>
        ...
        <div class="hs-overlay-open:mt-7 hs-overlay-open:opacity-100 hs-overlay-open:duration-500 mt-14 opacity-0 ease-out transition-all sm:max-w-lg sm:w-full m-3 sm:mx-auto">
            <div class="flex flex-col bg-white border shadow-sm rounded-xl pointer-events-auto">
                ...
                <div class="flex justify-end items-center gap-x-2 py-3 px-4 border-t">
                    ...
                    <button onclick="$('#form_update_prestasi').submit()" type="button" class="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent bg-red-600 text-white hover:bg-red-700 disabled:opacity-50 disabled:pointer-events-none">
                        Simpan Perubahan
                    </button>
                </div>
            </div>
